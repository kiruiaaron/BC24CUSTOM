page 50130 "Petty Cash Card"
{
    ...
    layout
    {
        area(content)
        {
            group(General)
            {
                ...
                field("Paid In Bank"; Rec."Paid In Bank")
                {
                    Editable = false;
                    ApplicationArea = All;
                }
    ...
    actions
    {
        area(processing)
        {
            ...
            action("Change To Imprest")
            {
                ApplicationArea = Basic, Suite;
                Caption = 'Change To Imprest';
                Image = ReOpen;
                Promoted = true;
                ...
                begin
                    ...
                    IF NOT CONFIRM('Are you sure you want to change this application to Imprest?') THEN
                        EXIT;
            ...
            group(Posting)
            {
                ...
                action("Print Petty Cash")
                {
                    Caption = 'Print Petty Cash';
                    Image = Print;
                    Promoted = true;
                    ...
                    begin
                        ...
                        IF ImprestHeader.FINDFIRST THEN BEGIN
                            REPORT.RUNMODAL(REPORT::"Petty Cash Voucher", TRUE, FALSE, ImprestHeader);
                        END;
                    end;
                }
            }
            group(IncomingDocument)
            {
                ...
                action("Paid in Bank Action")
                {
                    Image = Payment;
                    Promoted = true;
                    Caption = 'Paid In Bank';
                    PromotedCategory = Process;
                    PromotedIsBig = true;
                    ApplicationArea = All;
                    ...
                    trigger OnAction()
                    begin
                        IF NOT CONFIRM('Are you sure you want to Mark this Record as paid in Bank?') THEN
                            EXIT;
                        Rec.TESTFIELD(Status, Rec.Status::Approved);
                        Rec.TESTFIELD("Paid In Bank", FALSE);
                        Rec."Paid In Bank" := TRUE;
                        Rec."Date Paid" := CURRENTDATETIME;
                        Rec.MODIFY;
                        MESSAGE('Pettycash Marked Successfully');
                    end;
                }
            }
        }
