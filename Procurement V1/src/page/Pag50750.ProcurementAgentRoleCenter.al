...
page 50750 "Procurement Agent Role Center"
{
    Caption = 'Procurement Agent';
    PageType = RoleCenter;
    ...
    layout
    {
        area(rolecenter)
        {
            ...
            {
                ShowCaption = false;
                part(Control1907662708; "Procurement Agent Activities")
                {
    ...
    actions
    {
        ...
        area(embedding)
        {
            action("Purchase Requisitions")
            {
                ...
                ToolTip = 'Create Purchase requisitions';
            }
            action("Store Requisitions")
            {
                ApplicationArea = Basic, Suite;
                RunObject = page "Store Requisitions List";
                ToolTip = 'Create Store requisitions';
            }
            action("RFQ List")
            {
                ApplicationArea = Basic, Suite;
        ...
        area(sections)
        {
            ...
            group("Purchase Requisitions Group")
            {

                Caption = 'Purchase Requisitions';
                Image = Purchasing;
                action("Requisition Codes")
                {
                    ApplicationArea = Basic, Suite;
                    RunObject = page "Purchase Requisition Codes";
                }
                action("New Purchase Requisitions")
                {
                    ApplicationArea = Basic, Suite;
                    RunObject = page "Purchase Requisition List";
                ...
                {
                    ApplicationArea = Basic, Suite;
                    RunObject = page "Pend Purchase Requisition List";
                ...
                action("Approved Purchase Requisitions")
                {
                    ...
                    RunObject = page "App Purchase Requisition List";
                }
            }
            group("Store Requisitions Group")
            {
                Caption = 'Store Requisitions';
                action("New Requisitions")
                {
                    RunObject = page "Store Requisitions List";
                    ApplicationArea = Basic, Suite;
                }
                action("Requisitions Pending Approval")
                {
                    RunObject = page "Pend Store Requisitions List-s";
                    ApplicationArea = Basic, Suite;
                }
                action("Approved Requisitions")
                {
                    RunObject = page "App Store Requisitions List";
                    ApplicationArea = Basic, Suite;
                }
                action("posted Requsitions")
                {
