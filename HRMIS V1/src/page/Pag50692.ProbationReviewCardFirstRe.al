page 50692 "Probation Review Card-First Re"
{
    PageType = Card;
    SourceTable = 50328;
    SourceTableView = WHERE("Review Stage" = CONST("3-Month Review"));
    ...
    actions
    {
        area(processing)
        {
            action("Initial Meeting")
            {
                ...
                trigger OnAction()
                begin
                    IF CONFIRM('Are you sure you want to move this to Final Review?') THEN BEGIN
                        Rec."Review Stage" := Rec."Review Stage"::"6-Month Review";
                        Rec.MODIFY;
