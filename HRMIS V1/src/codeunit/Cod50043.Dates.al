codeunit 50043 Dates
{
    ...
    /// <param name="DateOfJoin">Date.</param>
    /// <returns>Return variable AgeString of type Text[45].</returns>
    ...
    begin
        ...
        IF DateOfBirth <> 0D/*AND (DateOfJoin <> 0D))*/ THEN BEGIN
            ...
            Month := 0;
            Year := 0;
            DueDate := DATE2DMY(TODAY, 3);
            //Age:=DueDate-yearB;
            //      Age := (yearB + CompInfo."Permanet Age" - Age) - DueDate;
            ...
            CASE (DateCat) OF
                1:
                    BEGIN
                        ...
                        AgeString := '%1';
                        AgeString := STRSUBSTNO(AgeString, Years);
                ...
                6:
                    BEGIN
                        ...
                        Years := yearJ - yearB;
                        AgeString := '%1';
                        AgeString := STRSUBSTNO(AgeString, Years, Month);
                    END;
                ELSE
                    AgeString := '';
            END;
    ...
    procedure DetermineAgeService(DateOfBirth: Date; DateOfJoin: Date) AgeString: Text[45]
    var
        dayB: Integer;
        monthB: Integer;
        yearB: Integer;
    ...
    begin
        IF ((DateOfBirth <> 0D) AND (DateOfJoin <> 0D)) THEN BEGIN
            ...
            CompInf.GET;
            DueDate := DATE2DMY(TODAY, 3);
            MonthsDue := DATE2DMY(TODAY, 2);
            Age := DueDate - yearB;
            Months := MonthsDue - monthB;
            //  Years := (yearB + CompInf."Permanet Age") - DueDate;
            DaysDue := DATE2DMY(TODAY, 1);
            Days := DaysDue - dayB;
            CASE (DateCat) OF
                1:
                    BEGIN
                        Year := yearJ - yearB;
                        IF Months >= monthB THEN
                            Month := Months - monthB
                        ELSE BEGIN
                            Month := (Months + 12) - monthB;
                            Year := Year - 1;
                        ...
                        ELSE
                            IF (dayJ < dayB) THEN BEGIN
                                Day := (DetermineDaysInMonth(Months, Years) + Days) - dayB;
                                Month := Month - 1;
                            END;

                        /*AgeString:= '%1  Years, %2  Months and #3## Days';
                        AgeString:= STRSUBSTNO(AgeString,Years,Months,Days);*/

                        AgeString := '%1  Years, %2  Months';
                        AgeString := STRSUBSTNO(AgeString, Years, Months);

                    END;

                2, 3, 7:
                    BEGIN
                        IF (Months <> monthB) THEN BEGIN
                            IF Months >= monthB THEN
                                Month := Months - monthB;
                        END;

                        IF (Days <> dayB) THEN BEGIN
                            IF (Days >= dayB) THEN
                                Day := Days - dayB
                            ELSE
                                IF (Days < dayB) THEN BEGIN
                        ...
                        /*AgeString:= '%1  Months %2 Days';
                        AgeString:= STRSUBSTNO(AgeString,Months,Days);*/
                        AgeString := '%1  Months';
                        AgeString := STRSUBSTNO(AgeString, Months);
                    END;
                4:
                    BEGIN
                        Years := yearJ - yearB;
    ...
    procedure pwdDetermineAgeService(DateOfBirth: Date; DateOfJoin: Date) AgeString: Text[45]
    var
        dayB: Integer;
        monthB: Integer;
        yearB: Integer;
            ...
            CASE (DateCat) OF
                1:
                    BEGIN
                        ...
                        AgeString := '%1  Years, %2  Months and #3## Days';
                        AgeString := STRSUBSTNO(AgeString, Years, Month, Day);
    ...
    procedure DetermineAgeService2(DateOfBirth: Date; DateOfJoin: Date) AgeString: Text[45]
    var
        dayB: Integer;
        monthB: Integer;
        yearB: Integer;
    ...
    begin
        IF ((DateOfBirth <> 0D) AND (DateOfJoin <> 0D)) THEN BEGIN
            CompInf.GET;
            //    Years := CompInf."Permanet Age";
            //  EVALUATE(retireagef, FORMAT(CompInf."Permanet Age") + 'Y');

            retirementdate := CALCDATE(retireagef, DateOfBirth);
            Years := retirementdate - TODAY;
            Year := Years DIV 365;
            Months := Years MOD 365;
            Month := Months DIV 30;
    ...
    procedure pwdDetermineAgeService2(DateOfBirth: Date; DateOfJoin: Date) AgeString: Text[45]
    var
        dayB: Integer;
        monthB: Integer;
        yearB: Integer;
    ...
    begin
        IF ((DateOfBirth <> 0D) AND (DateOfJoin <> 0D)) THEN BEGIN
            CompInf.GET;
            // Years := CompInf."PWD Age";
            //  EVALUATE(retireagef, FORMAT(CompInf."PWD Age") + 'Y');
