codeunit 51151 "Calculate All Payrolls"
{
        ...
        IF ConfirmRun THEN BEGIN
            ...
            REPEAT
                ...
                IF Emp."Customer No." <> '' THEN BEGIN
                    OutStandingAmt := 0;
                    Cust.RESET;
                    Cust.SETCURRENTKEY("No.", "Currency Code");
                    Cust.SETRANGE(Cust."No.", Emp."Customer No.");
                    // Cust.SETFILTER(Cust.on, '%1', '');
                    IF Cust.FINDSET THEN
                        REPEAT
                            Cust.CALCFIELDS(Cust."Reminder Amounts");
                            OutStandingAmt += Cust."Reminder Amounts (LCY)";
                        UNTIL Cust.NEXT = 0;
                ...
                REPEAT
                    ...
                    IF (NOT SchemeControlTmp."Annualize TAX") AND (NOT SchemeControlTmp."Annualize Relief") THEN BEGIN
                        ...
                        IF (SchemeControlTmp."Special Allowance") AND (NOT SchemeControlTmp."Special Allowance Calculated")
                          THEN
                            GetSpecialAllowance(PayrollHeader); //IGS Payroll: Special Allowances

                        IF (SchemeControlTmp."Special Payment") AND (NOT SchemeControlTmp."Special Payment Calculated")
                          THEN
                            GetSpecialPayments(PayrollHeader); //IGS Payroll: Special Payments
                    ...
                    END ELSE BEGIN
                        ...
                        IF PeriodRec."Annualize TAX" THEN BEGIN//V.6.1.65_10SEP10 >>
                            ...
                            IF (SchemeControlTmp."Special Allowance") AND (NOT SchemeControlTmp."Special Allowance Calculated")
                               THEN
                                GetSpecialAllowance(PayrollHeader);

                            IF (SchemeControlTmp."Special Payment") AND (NOT SchemeControlTmp."Special Payment Calculated")
                               THEN
                                GetSpecialPayments(PayrollHeader);
    ...
    var
        ...
        SchemeValueTmp: Record "Calculation Scheme";
        EntryLines: Record "Payroll Entry";
        EntryLinesTMP: Record "Payroll Entry";
        PayrollLinesTmp: Record "Payroll Lines";
        EDDefinitionTmp: Record "ED Definitions";
    ...
    procedure GetBasicPay(var Header: Record 51159)
    var
        EmployeeRec: Record Employee;
        PeriodRec: Record Periods;
    ...
    procedure LookUpEth(var Header: Record 51159)
    var
        LookUpHeader: Record 51162;
        LookUpLine: Record 51163;
        Relief: Decimal;
